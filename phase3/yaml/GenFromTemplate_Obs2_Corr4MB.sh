...
templateFile="config_SideBand_Corr4MB_Obs2_Template.yaml"
...
do
  ...
  PTHIGH=${PTHIGHS[$PTBIN]}
	...
	do
		mkdir -p Obs2/PtBin$PTBIN/Cent$CENTBIN/
		CENTLABEL=$CENTBIN
		if [ $CENTBIN = 0 ]
		then
			CENTLABEL="0-10%"
        ...
        then
          SBBkgSels=(0)
				...
				do
					for EPBIN in ${EPBINS[@]}
					do
#						NewFileName=Obs2/PtBin$PTBIN/Cent$CENTBIN/PurChoice$PURCHOICE/config_P2_Corr4MB_SBSel$BkgSel""_Cent$CENTBIN""_EP$EPBIN""_PurChoice$PURCHOICE.yaml
#						echo "Creating file $NewFileName"
#						cat $templateFile | sed -e "s/\$BKGSEL/$BkgSel/g" -e "s/\$PTBIN/$PTBIN/g" -e "s/\$CENTBIN/$CENTBIN/g" -e "s/\$EPBIN/$EPBIN/g" -e "s/\$PTHIGH/$PTHIGH/g" -e "s/\$PTLOW/$PTLOW/g"  -e "s/\$CENTLABEL/$CENTLABEL/g" -e "s/\$PURITYCHOICE/$PURCHOICE/g" > $NewFileName
            NewFileName=Obs2/PtBin$PTBIN/Cent$CENTBIN/PurChoice$PURCHOICE/config_P2_Corr4MB_SBSel$BkgSel""_FitFunc$FITFUNC""_PtBin$PTBIN""_Cent$CENTBIN""_EP$EPBIN""_PurChoice$PURCHOICE.yaml
            echo "Creating file $NewFileName"
