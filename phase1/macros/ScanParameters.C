...
void ScanParameters(int bTrigger) { 
	int iPtBin = 1;
	if (bTrigger == 1) {
		sTrigger = "GA";
		iPtBin = 4;
...
void ScanParametersCent(int iCentBin, int iPtBin) {
	...
	// Load the files
	TString sFileFormat = "output/T40/ThetaLambdaScan/Lambda0%d/Theta0%d/%s_HadCorr/Cent%d/EP-1/SecondAnalysis.root";
	...
	for (int iLambdaBin : fLambdaBins) {
		...
		for (int iThetaBin : fThetaBins) {
			...
			TString sParamScanFileName = Form(sFileFormat.Data(),iLambdaBin,iThetaBin,sTrigger.Data(),iCentBin);
